...
odp_config (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  vlib_buffer_add_mem_range (vm, info.min_data_addr,
			     info.max_data_addr - info.min_data_addr + 1);

  return 0;
}

VLIB_CONFIG_FUNCTION (odp_config, "odp-late");
...
static clib_error_t *
odp_early_config (vlib_main_t * vm, unformat_input_t * input)
{
  char *param = NULL;
...
VLIB_EARLY_CONFIG_FUNCTION (odp_early_config, "odp");
